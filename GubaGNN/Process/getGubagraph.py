...
def constructMat(tree):
    ...
    matrix=np.zeros([len(index2node),len(index2node)]) # Adjacency marix
    raw=[]
    col=[]
    x_word=[]
    x_index=[]
...
def main():
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez(os.path.join(cwd,'data/Gubagraph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
        x_word, x_index, tree, rootfeat, rootindex = constructMat(event)
        x_x = getfeature(x_word, x_index)
